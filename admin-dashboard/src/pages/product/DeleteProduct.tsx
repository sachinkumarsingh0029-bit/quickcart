...
function DeleteProduct({ isOpen, closeModal, setLoading, productId }: any) {
  ...
  const handleDeleteAccount = async () => {
    try {
      ...
      CreateToast("deleteuser", "Product deleted successfully", "success");
      closeModal();
      navigate("/product");
    } catch (error: any) {
      CreateToast(
        "deleteuser",
        "Something went wrong while deleteing...",
        "error"
      );
    }
  };
            ...
            <div className="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl rtl:text-right dark:bg-gray-900 border-gray-800 sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6 text-black dark:text-white">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                ...
                <p className="text-sm text-red-500 mt-4">
                  Please be aware that deleting this product will permanently
                  remove all associated data and cannot be retrieved. Proceed

...
import { deleteSeller, rejectSeller } from "../../../api/seller";

function RejectSellerRequest({ isOpen, closeModal, sellerId }: any) {
  const [reason, setReason] = useState("");
  const navigate = useNavigate();

  // reset password
  const handleRejectSellerRequest = async () => {
    try {
      if (reason === "") return;
      const data = {
        reason,
      };
      const result = await rejectSeller(sellerId, data);
      console.log(result);
      if (result.status === "success") {
        CreateToast("rejectseller", result.message, "success");
        closeModal();
            ...
            <div className="relative inline-block px-4 pt-5 pb-4 overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl rtl:text-right dark:bg-gray-900 border-gray-800 sm:my-8 sm:align-middle sm:max-w-sm sm:w-full sm:p-6 text-black dark:text-white">
              <div className="grid w-full max-w-sm items-center gap-1.5">
                ...
                <p className="text-sm text-red-500 mt-4">
                  Warning: Please be aware that if you proceed with rejecting
                  this seller account request.
                </p>
              </div>
              ...
              <div className="mt-5 sm:flex sm:items-center sm:justify-end">
                <div className="sm:flex sm:items-center ">
                  ...
                  <button
                    type="button"
                    onClick={handleRejectSellerRequest}
...
export default RejectSellerRequest;
